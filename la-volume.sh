...
#flirt + xfm_apply
/usr/local/fsl/bin/flirt -in /export/higgs/john.chan/ref_brains/${pre}_brain.nii.gz -ref /export/higgs/john.chan/LA_brains/${pre}/brain.nii.gz -out /export/higgs/john.chan/ref_brains/${pre}_brain_diff.nii.gz -omat /export/higgs/john.chan/ref_brains/${pre}_brain_diff.mat -bins 256 -cost normmi -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12  -interp trilinear
...
#create density mask
track_density_mask ${pre}_network.csv.trk LA_mask.nii ${pre}_networkDenMask.nii
...
fslmaths ${pre}_networkDenMask_bin.nii -mul LA_mask.nii ${pre}_LA_intersect.nii
